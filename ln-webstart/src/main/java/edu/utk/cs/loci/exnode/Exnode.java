...
public class Exnode extends MetadataContainer
{
    ...
    public List<Mapping> getContainingMappings( long offset, long length )
    {
    ...
    {
        Iterator<Mapping> mappings = getAllMappings().iterator();
        Mapping m;
        long length = 0;
        long offset = -1;
    ...
    public SortedSet<Long> getTransferBoundaries(List<Mapping> mappingsList,
        int maxTransferSize )
    {
        SortedSet<Long> transferBoundaries = new TreeSet<Long>();

        for ( Iterator<Mapping> i = mappingsList.iterator(); i.hasNext(); )
        {
        ...
        int transferSize, int numThreads ) throws Exception
    {
        ...
        Job job = new ReadJob( this, readOffset, readLength, file, offset );
        mainQueue.add( job );
